...
提供了一些开发环境，以便您可以选择更适合您的环境；您将使用一些调试技巧，这将在开发您自己的代码时给您一个良好的开端，并让您更好地了解如何消除应用程序中的 bug。
...
整洁的应用程序就是这样完成的。现在，每征服一个星球，我们只需要编辑`countWorlds`变量。反过来，每次我们修改这个变量，HTML 就会自动更新。

恭喜您，您已经完成了进入 Vue 世界的第一步，现在您已经能够使用反应式数据绑定和字符串插值来构建简单的交互式应用程序了。
...
在 Vue 中，大多数数据都是被动的。实际上，这意味着如果我们的视图模型发生了变化，我们将立即看到结果。这是让你专注于应用程序本身，抛开所有的绘图逻辑。在这个食谱中，我们也要承认这个系统的一些局限性。
...
每个应用程序的一个基本部分是与用户的交互。Vue 有速记来拦截大多数用户事件，并将它们连接到相关的动作。
...
试用一下这个应用程序，注意保持文本框同步不需要任何处理程序。每次`toastedBreads`更新，文字也会更新；相反，每次你写一个数字，数量也会更新。
...
有一系列的网站，比如 JSFiddle，可以让你直接从浏览器(CodePen 和 JS Bin 等)编写 Vue 应用程序，这些网站非常适合测试新功能和尝试本书中的方法。另一方面，他们在代码组织方面太受限制，无法开发更多的东西。在本章的第一个配方中，使用了这种开发风格，所以请参考它来学习如何只使用浏览器进行开发。总的来说，你应该根据你正在开发的东西，把你通过这种方法学习到的东西转移到更结构化的项目中去。
...
这种方法类似于 JSFiddle:我们有一个 HTML 部分、一个 JavaScript 部分和一个 CSS 部分。我们正在控制一切。此外，这样我们可以使用 Vue 开发工具(查看使用 Vue 开发工具对您的应用程序进行 X 射线检查的方法*以了解相关介绍)。*
...
2.  一旦脚手架完成，键入`npm install`。这将负责下载和安装我们编写 Vue 应用程序所需的所有 npm 包。

在这之后，您就已经有了一个正常运行的演示应用程序。

3.  键入`npm run dev`运行您的应用程序。进一步的指示，将出现在屏幕上，并会告诉你访问一个特定的网址，但有一个很好的机会，你的浏览器将自动打开。
4.  将浏览器指向指定的地址。您应该可以马上看到演示应用程序。

探索由`vue-cli`创建的源文件，你会发现两个值得注意的文件。第一个文件是应用程序的入口点，`src/main.js`。它将包含如下内容:
...
您将找到的`App.vue`文件是一种自包含的方式，您可以用它来编写 Vue 组件。您将在其他菜谱中找到更多关于组件的内容，但是现在，您可以将它视为一种可以进一步划分应用程序以使其更加有序的方式。
...
在`<template>`标签中我们放入 HTML，在`<script>`标签中放入 JavaScript 代码，我们使用`<style>`标签为我们的应用程序添加一些样式。
...
如果我们试图在 Vue 应用程序中运行 HTML，它会抱怨`[Vue warn]: Failed to resolve filter: capitalize`。
...
3.  运行你的应用程序，注意`cat`对象是如何完美输出的，就像`JSON.stringify`一样。
...
让您的迁移有一个良好的开端；Chris Fitz(Vue 核心团队的成员)创建了一个小助手应用程序，它将扫描您的代码并指导您进行迁移:

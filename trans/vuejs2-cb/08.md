...
现在我们的应用程序工作了，你可以看到我们将 0.93 美元存入一个利率为 2.25%的银行账户，然后休眠 1000 年(43 亿美元！):
...
Webpack 允许您将项目打包成缩小的 JavaScript 文件。然后，您可以分发这些文件或自己使用它们。当您使用`vue-cli`自带的内置模板时，Webpack 被配置为用它构建一个完整的工作应用程序。有时我们想要构建一个库来发布或在另一个项目中使用。在这个菜谱中，您将调整 Webpack 模板的默认配置来发布一个组件。
...
为了手动测试我们的应用程序，我们可以通过以下方式修改`main.js`文件(突出显示的文本是修改后的代码):
...
它最初做的是在一个`js`目录中创建一系列输出文件。方括号内有变量；我们不需要它们，因为我们的应用程序只有一个独立的模块，我们称之为 *shaker* 。我们需要获得以下代码:
...
Webpack 是一个组织代码和依赖项的工具。此外，它为您提供了一种开发和构建 JavaScript 文件的方法，这些文件嵌入了我们传递给它们的所有依赖项和模块。我们将在这个菜谱中使用它来构建一个小的 Vue 应用程序，并将所有内容打包到一个文件中。
...
如果你现在尝试用`npm run dev`命令运行你的应用程序，你会看到布尔玛模态对话框的辉煌:
...
您需要的最后一个应用程序文件是一个 JavaScript 文件，它将包含我们的 Vue 根实例。在同一个目录下创建，命名为`index.js`；把以下内容放进去:
...
现在您需要创建一个文件`webpack.config.js`，告诉 Webpack 一些事情。第一件事是我们的应用程序的入口点(`index.js`)和我们想要放置编译文件的地方:
...
点击加号按钮会使计数器上升，但是这个应用程序的风格呢？让我们把它变得更有吸引力。

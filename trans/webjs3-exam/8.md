...
![Figure 8.1 – Diagram of the chat app's architecture ](image/Figure_8.1_B14405.jpg)
...
它的内容与`AddChatroomForm.vue`文件相同，但`ChatroomForm`上的`edit`属性设置为`true`，而`id`属性设置为我们从 Vue 路由器获得的 URL 中的`id`URL 参数。
...
![Figure 8.2 – Screenshot of the Chatroom ](image/Figure_8.2_B14405.jpg)

图 8.2–聊天室的屏幕截图

下面的屏幕截图是 Laravel Echo 服务器正在工作。我们应该看到所广播事件的名称及其发送的频道：

![Figure 8.3 – The output from Redis when chat events are sent to the frontend ](image/Figure_8.3_B14405.jpg)

图 8.3–聊天事件发送到前端时 Redis 的输出

下面的截图是队列事件的日志：

![Figure 8.4 – Output of Laravel events ](image/Figure_8.4_B14405.jpg)

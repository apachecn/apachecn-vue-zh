...
现在我们已经完成了后端应用程序，我们可以继续在前端工作了。因为我们之前已经在`admin-frontend`文件夹中为管理员前端创建了 Vue 3 项目，所以我们只需安装所需的软件包，然后再编写代码。我们需要`graphql-request`GraphQL 包和 GraphQL 客户端库，以及 VeeValidate、Vue 路由、Axios 和 Yup 包。
...
在`src/main.js`中，我们编写以下代码将路由添加到我们的应用程序中：
...
在`src/main.js`中，我们添加以下代码，将路由和 Vuex 商店添加到我们的应用程序中：

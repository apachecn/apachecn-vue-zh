...
`grapgqlHTTP`函数在我们传入一组选项后返回一个中间件。我们为 GraphQLAPI 设置模式。`rootValue`属性有一个包含所有解析程序的对象。解析程序名称应与`Query`和`Mutation`类型中指定的名称匹配。`graphiql`属性设置为`true`，这样我们可以在浏览器中转到`/graphql`页面时使用可用的 GraphiQL web 应用程序。
...
![Figure 7.1 – GraphiQL with ModHeader extension ](image/Figure_7.1_B14405.jpg)
...
![Figure 7.2 – Admin login screen ](image/Figure_7.2_B14405.jpg)
...
![Figure 7.3 – Orders page: admin view ](image/Figure_7.3_B14405.jpg)
...
![Figure 7.4 – Shop items page: admin view ](image/Figure_7.4_B14405.jpg)
...
最后，在`package.json`中，我们更改服务器脚本，从不同的端口为应用程序提供服务，这样它就不会与前端冲突。为此，我们编写以下代码：
...
![Figure 7.5 – Order form: admin view ](image/Figure_7.5_B14405.jpg)

...
为了清晰和避免混淆，我们将在讨论 VuePress 自定义主题的章节中讨论与主题相关的配置。类似地，我们将在有关 VuePress 中标记的章节中返回讨论与标记相关的配置值。
...
这显示了站点元描述。因此，在 HTML 页面中，它在`<meta>`标记中呈现：
...
这包括当页面以 HTML 呈现时在`<head>`标记内呈现的附加数据。通常，它采用包含在`[`和`]`方括号内的内容的合并形式。
...
好了，就这样！现在，您已经了解了 VuePress 中所有必需配置实体的基本细节。再次，我们省略了与主题配置相关的配置术语，以及标记，我们将在后面的章节中讨论。
...
此代码将确保引用坚持到指定的文件，而不管您使用 VuePress 的基本 URL 是什么。您可以使用它来处理公共文件，甚至可以在主题组件和标记文件中使用它。

...
*   在 VuePress 中构建站点
*   如何创建Markdown 文件并运行 VuePress 开发引擎
...
此外，您还需要使用基本的Markdown 文件来输入内容。大多数 web 开发人员都很清楚降价，并且大多数开发人员定期使用降价。如果您碰巧不知道什么是Markdown，或者不习惯使用它，请参阅本书的[第 5 章](5.html)、*在 VuePress*中使用Markdown。也就是说，在本章中，我们将只触及降价元素的表面，例如段落、标题和链接。因此，就目前而言，即使是对降价的基本认识也已经足够了。
...
确保在安装 VuePress 的确切目录中运行[T0]命令。所有文件都将放置并创建在其中，包括Markdown 文件、任何媒体元素以及任何 JS 配置文件。
...
值得注意的是，整个文件仅由前端内容组成。当我们讨论主题开发时，我们将在[第 5 章](5.html)*中使用 VuePress*中的Markdown以及[第 6 章](6.html)【VuePress 中的主题开发】中详细了解 YAML 前端内容。
...
1.  我们将内容添加为降价文件
2.  VuePress 从这些Markdown 文件中转换 HTML 文件，只要所述文件的格式有效
...
同样，我们也可以添加[T0]文件。事实上，我们可以添加任意多的页面。我们需要做的就是添加Markdown 文件，并通知 VuePress 正确的位置。
...
此命令将提取我们的站点元数据并编译信息。然后，它将根据Markdown内容呈现静态 HTML 页面，如下所示：
...
值得指出的是，VuePress 根据我们创建的Markdown 文件生成 HTML 文件。它还将所有媒体元素、JS 文件和 CSS 文件放置在`.vuepress/dist`路径的资产子目录中。`/img`目录由图像组成，`/js`子目录由 JS 文件组成，顾名思义

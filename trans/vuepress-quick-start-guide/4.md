...
此外，您还需要使用基本的标记文件来输入内容。大多数 web 开发人员都很清楚降价，并且大多数开发人员定期使用降价。如果您碰巧不知道什么是标记，或者不习惯使用它，请参阅本书的[第 5 章](5.html)、*在 VuePress*中使用标记。也就是说，在本章中，我们将只触及降价元素的表面，例如段落、标题和链接。因此，就目前而言，即使是对降价的基本认识也已经足够了。
...
![](assets/c9a66607-bc1e-4917-b633-1a7a0535ad3d.png)
...
![](assets/551ae3e7-d7b5-4c1e-b312-d3d0dc2e3121.png)
...
![](assets/ee5f242c-6278-4a90-9bc5-cab24e6b09a3.png)

现在，我们可以在 web 浏览器中打开提供的 URL，如上面的屏幕截图所示，当我们对文件结构进行更改时，页面将自动刷新，以在保存更改时显示实时更改。只要开发引擎正在运行（即，终端处于打开状态），我们就可以在保存内容时查看实时更改。

确保在安装 VuePress 的确切目录中运行[T0]命令。所有文件都将放置并创建在其中，包括标记文件、任何媒体元素以及任何 JS 配置文件。
...
![](assets/4e0593f4-7cde-4304-b19e-c86543203743.png)
...
![](assets/c5bad76e-fb41-41c2-998f-a9507214a166.png)
...
![](assets/fcf4fa9d-5480-40e9-a309-8a2754710737.png)
...
![](assets/3af276f0-ad11-4568-938c-977704284048.png)

值得注意的是，整个文件仅由前端内容组成。当我们讨论主题开发时，我们将在[第 5 章](5.html)*中使用 VuePress*中的标记以及[第 6 章](6.html)【VuePress 中的主题开发】中详细了解 YAML 前端内容。
...
1.  我们将内容添加为降价文件
2.  VuePress 从这些标记文件中转换 HTML 文件，只要所述文件的格式有效
...
![](assets/c6b20e73-e679-46f0-a087-3132460aa337.png)

这就是输出应该显示的内容（添加此文件后，请确保再次运行[T0]命令）：

![](assets/35b9da51-5822-4988-b9d9-e0565c64c76f.png)
...
*   更重要的是，Markdown 中的每一个 H1、H2、H3 和其他类似标题标记都被视为 HTML 中的锚链接。您可以将光标悬停在任何标题上，并注意自动为其生成锚定链接。

同样，我们也可以添加[T0]文件。事实上，我们可以添加任意多的页面。我们需要做的就是添加标记文件，并通知 VuePress 正确的位置。
...
![](assets/bbdc5f3e-65f2-4286-ad46-92622140d817.png)
...
![](assets/e6eb3268-ebed-4f05-a1d8-377a58814bfd.png)
...
![](assets/677e12c9-5d3c-4a43-b9d0-fd7b034b5d26.png)
...
此命令将提取我们的站点元数据并编译信息。然后，它将根据标记内容呈现静态 HTML 页面，如下所示：

![](assets/8602d2ef-e97a-4255-972b-a14502d14517.png)

生成的内容默认放置在`.vuepress`目录下的 dist 目录中。因此，我们的 VuePress 站点的一般目录结构如下：

![](assets/2894978d-8b61-404a-bef2-4a937ae403b1.png)

值得指出的是，VuePress 根据我们创建的标记文件生成 HTML 文件。它还将所有媒体元素、JS 文件和 CSS 文件放置在`.vuepress/dist`路径的资产子目录中。`/img`目录由图像组成，`/js`子目录由 JS 文件组成，顾名思义

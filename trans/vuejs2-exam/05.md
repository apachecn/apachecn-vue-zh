...
我们的 Dropbox web 应用现在完全可以通过结构列表和面包屑导航，我们现在可以添加和更新浏览器 URL，以便快速访问和共享文件夹。我们可以通过两种方式做到这一点:我们可以更新散列，例如`www.domain.com/#/images/holiday/summer`，或者我们可以将所有路径重定向到单个页面，并在 URL 中没有散列的情况下处理路由。

...
我们将学习如何使用各种`vue-devtools`，以便深入了解 Vue 组件，了解我们的数据和应用发生了什么变化。
...
当 Vue 团队意识到开发人员在创建和管理其应用时遇到问题时，他们看到了一个机会，可以创建一个可以帮助世界各地开发人员的工具。有鉴于此，Vue CLI 项目诞生了。
...
4.  在这一点上，我们将被问及我们希望为我们的项目提供哪些功能。这些特性是一些 Vue 插件，如 Vuex 或路由（Vue 路由）、测试器、linter 等。对于本项目，我们将选择`CSS Pre-processors`并按*进入*继续：
...
5.  CLI 将要求您选择用于启动应用的 Vue 版本。我们将在这里选择`3.x (Preview)`。按*进入*继续：
...
Vue UI 工具中的每个`npm`脚本都称为一个任务，在这些任务上，您可以收集实时统计信息，如资产、模块和依赖项的大小；错误或警告的数量；以及用于微调应用的更深入的网络数据。
...
Vue 应用是各种组件的组合，由 Vue 框架绑定在一起并进行编排。了解如何制作组件非常重要。每个构件就像墙上的一块砖，需要以一种在放置时不需要在其周围以不同方式重塑其他砖的方式制作。在本食谱中，我们将学习如何在遵循一些重要原则的同时制作一个基本组件，这些原则侧重于组织和干净的代码。
...
6.  在`main.js`文件中，我们需要导入`style.css`文件，以便将其包含在 Vue 应用中：
...
制作待办事项应用涉及到一些基本原则——它必须包含一个任务列表，任务可以标记为完成和撤消，列表可以过滤和排序。现在，我们将学习如何接受任务并将其添加到任务列表中。
...
向组件中添加样式是一种很好的做法，因为它允许您更清楚地向用户显示正在发生的事情。通过这样做，您可以向用户显示视觉响应，并让他们更好地体验您的应用。
...
每次应用中的一个新项目标记为“完成”时，`displayList`属性就会更新并触发组件的重新招标。
...
在本教程中，我们将学习更多关于如何使用各种 devtool 来了解更多关于我们的应用的信息，以及如何使用它们来帮助调试过程。
...
在开发任何 Vue 应用时，使用手边的`vue-devtools`进行开发始终是一种良好的做法。

按照以下步骤了解如何使用`vue-devtools`以及如何正确调试 Vue 应用：

1.  要输入`vue-devtools`，您需要将其安装在浏览器中，因此请查看本配方的*准备*部分，以获取指向 Chrome 或 Firefox 扩展的链接。在 Vue 开发应用中，输入浏览器开发人员检查器模式。将出现一个名为 Vue 的新选项卡：

![](img/22f261fe-85aa-4a66-88c4-3ee24d533afd.png)

2.  您将看到的第一个选项卡是“组件”选项卡。此选项卡显示应用组件树。如果您单击某个组件，您将能够看到所有可用数据、计算属性以及由插件（如`vuelidate`、`vue-router`或`vuex`）注入的额外数据。您可以编辑此数据以实时查看应用中的更改：
...
4.  第三个选项卡专用于应用中的事件发射器。应用中发出的所有事件都将显示在此处。您可以通过单击来检查发出的事件。通过这样做，您可以看到事件的名称、类型、事件的来源（在本例中，它是一个组件）和有效负载：

![](img/b9ec3e07-899e-4f60-8f38-369a33d90799.png)

5.  第四个选项卡专用于 vue 路由插件。在那里，您可以查看其导航历史记录，以及传递到新路由的所有元数据。在这里，您可以检查应用中的所有可用路由：

![](img/3c2e94ba-9624-45ae-b10b-112e01afada3.png)

6.  第五个选项卡是性能选项卡。在这里，您可以检查组件的加载时间和应用运行时的每秒帧数，以了解实时发生的事件。以下屏幕截图显示了当前应用和选定组件的每秒当前帧数：
...
8.  最后一个选项卡是`vue-devtools`的刷新按钮。有时，当`hot-module-reload`发生时，或者当应用组件树中发生一些复杂事件时，扩展可能会丢失正在发生的事情。此按钮强制扩展重新加载并再次读取 Vue 应用状态。

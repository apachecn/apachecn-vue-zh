...
2.  在`/assets/`目录的`/css/`文件夹中创建一个`main.scss`文件，导入动作界面如下：
...
如果您想将本地样式应用于特定页面或布局，这种方法是好的且易于管理的。您应该在`lang`属性之前添加`scoped`属性，以便本地样式在本地应用于特定页面，并且不会干扰其他页面中的样式。但是，如果有多个页面和布局共享一个公共样式，则应在项目的`/assets/`目录中全局创建样式。因此，让我们通过以下步骤了解如何使用较少的资源创建全局样式：
...
2.  在`/assets/`目录中创建`main.less`文件，并添加以下样式：
...
同样，我们在插件文件中组织了 CSS 资源，而不是通过 Nuxt 配置文件中的`css`选项全局注册它。但是，如果您想全局覆盖这些 UI 框架和库中的某些样式，则可以通过在`css`选项中全局注册 CSS 资源，然后在`/assets/`目录中存储的 CSS 文件中注册自定义样式，从而更容易覆盖这些样式。
...
在下一章中，我们将介绍 Nuxt 中的视图、路由和转换。您将创建自定义页面、路由和 CSS 转换，并学习如何使用`/assets/`目录为图像和字体等资产提供服务。此外，您还将了解如何自定义默认布局并在`/layouts/`目录中添加新布局。给你一个简单的例子，你可以从 Nuxt 网站上了解到所有这些功能。因此，让我们在下一章中进一步探讨 Nuxt！******
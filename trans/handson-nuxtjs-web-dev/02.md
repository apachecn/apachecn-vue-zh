...
在即将到来的章节中，您将学习如何安装自定义 UI 框架、库和工具，如 ZURB 基金会、运动 UI、JQuery UI 和应用的更少 CSS。您将编写一些基本代码来设置索引页的样式，并向其中添加一些动画。您还将开始使用我们在本章中刚刚介绍的一些目录，例img/`、`/plugins/`和`/pages/`目录，来开发您的 Nuxt 应用。**
...
4.  使用`new`操作符创建路由器实例，并传入`routes`常量：
...
4.  在将路由器注入 Vue 根实例之前，定义路由器实例并传入前面的嵌套路由，如下所示：
...
![](assets/68280e28-fc1c-49b0-8bfa-422ee8202972.png)
...
![](assets/64eacc73-de5b-460b-8d5b-29af57a3c3c8.png)
...
在 Nuxt 中，不再需要`<transition>`组件。默认情况下，它是为您添加的，因此您只需在任何特定页面的`/assets/`目录或`<style>`块中创建转换。`pageTransition`属性在 Nuxt 配置文件中用于设置页面转换的默认属性。Nuxt 中转换属性的默认值如下所示：
...
1.  在`/assets/`目录中创建一个`transition.css`文件，并向其添加以下转换：
...
3.  在`/assets/`目录中创建一个`transition.css`文件，并向其添加以下转换：

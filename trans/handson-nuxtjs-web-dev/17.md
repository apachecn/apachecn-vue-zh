...
让我们浏览一下这段代码，了解它的作用。在这里，您可以看到，我们正在为 RejectDB 客户机驱动程序使用一个定制的顶级名称空间`rdb`，而不是您在`localhost:8080`上练习的`r`名称空间。此外，在我们的应用程序中使用 RejectDB 客户端驱动程序时，我们必须始终使用 RejectDB 服务器连接调用 ReQL 命令末尾的`run`方法来构造查询并将其传递到服务器上执行。

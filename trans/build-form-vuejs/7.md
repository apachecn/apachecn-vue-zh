...
目前，我们的表格（如前所述）是硬编码的。开始使其成为动态表单所需的第一步是，每次需要添加新字段时，都不需要将`BaseInput`或`BaseSelect`直接添加到`App.vue`文件中。这意味着我们将需要某种有组织的结构，或模式，来表示我们试图为我们的形式完成什么。因为我们使用的是 JavaScript，所以最合乎逻辑的方法是使用 JSON 对象格式。这将使以后我们更容易进一步，让我们的模拟 API 直接将信息提供给表单。

...
这里，我们使用了新的 JavaScript 字符串，带有```js字符（后引号）。除其他外，它允许我们编写跨多行的文本，而无需编写冗长的字符串连接。
...
3.  在`.turn-counter`元素中添加箭头图像，并使用[第 2 章](2.html)、*Markdown笔记本*中使用的`currentPlayerIndex`道具和`v-bind`速记添加动态类：

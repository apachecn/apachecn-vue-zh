...
在上一章中，我们实现了 ProFitOro 应用程序的主要功能–Pomodoro 定时器。我们甚至添加了一个硬编码的训练，这样我们可以在休息时进行锻炼。事实上，我已经开始使用 ProFitOro 了。当我写这些话的时候，Pomodoro钟倒计时–*滴答*。
...
因此，现在我们必须将 Vuex 存储连接到 Firebase数据库。我们可以使用本机 Firebase API 将状态数据绑定到数据库数据，但是如果有人已经为我们这样做了，我们为什么还要处理承诺之类的事情呢？这个人叫 Eduardo，他为 Vuex（[创建了 Vuexfire–Firebase绑定 https://github.com/posva/vuexfire](https://github.com/posva/vuexfire) ）。如果你在*弗罗茨瓦夫*的*vueconf2017 会议*，你可能还记得这个家伙：
